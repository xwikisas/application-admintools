...
class ConfigurationEncodingHealthCheckTest
{
    ...
    void checkNullJSON() throws Exception
    {
        ...
        assertEquals("Configuration encoding could not be detected!", logCapture.getMessage(1));
    }
    ...
    void checkUnsafeEncoding() throws Exception
    {
        ...
        assertEquals("adminTools.dashboard.healthcheck.security.xwiki.config.warn",
            configurationEncodingHealthCheck.check().getMessage());
        assertEquals("[XWiki configuration] encoding is [ISO-8859-1], but should be UTF-8!", logCapture.getMessage(0));
    }
}
